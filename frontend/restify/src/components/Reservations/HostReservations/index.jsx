...
function HostReservations(props) {
  ...
  useEffect(() => {
    const getReservations = async () => {
      ...
      const url = "http://localhost:8000/properties/reservations/host/?" + new URLSearchParams({
        page: page
      });
  ...
  return <div className="d-flex flex-column align-content-center">
    {
      reservations.length > 0
        ? reservations.map((item, index) => {
          return <ReservationCard key={index} index={index} reservation={item} isHost={true} updateReservation={updateReservations} />
        })
        : <div></div>
    }
    {
